...
public interface DependencyRepository extends JpaRepository<Dependency, String> {
	...
	List<Dependency> findByRequirementIdIncludeProposed(Collection<String> ids);
	
	@Query("SELECT DISTINCT dep FROM Dependency dep WHERE (dep.fromid IN (?1) OR dep.toid IN (?1)) AND (dep.status = 1)")
	List<Dependency> findByRequirementIdExcludeProposed(Collection<String> ids);
	...
	List<Dependency> findByRequirementIdWithParams(Collection<String> ids, Double scoreTreshold, Boolean includeProposed, Boolean proposedOnly,
